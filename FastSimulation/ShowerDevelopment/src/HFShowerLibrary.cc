...
#include "FastSimulation/ShowerDevelopment/interface/HFShowerLibrary.h"
...
HFShowerLibrary::HFShowerLibrary(edm::ParameterSet const & p) : fibre(0),hf(0),
							  	emBranch(0),
								hadBranch(0),
                                                          numberingScheme(0), 
                                                         numberingFromDDD(0), 
...
void const HFShowerLibrary::initHFShowerLibrary(const edm::EventSetup& iSetup) {
  ...
  initRun();  
  fibre = new HFFibre(name, *cpv, cFibre);
...
void HFShowerLibrary::recoHFShowerLibrary(const FSimTrack& myTrack) {
  ...
  G4ThreeVector vertex( 10.*myTrack.vfcalEntrance().vertex().X(),
                        ...
                        10.*myTrack.vfcalEntrance().vertex().Z()); // in [mm]
...
bool HFShowerLibrary::isItinFidVolume (G4ThreeVector& hitPoint) {
  bool flag = true;
  if (applyFidCut) {
...
std::vector<HFShowerLibrary::Hit> HFShowerLibrary::getHits(const G4ThreeVector & hitPoint,
